...
- Creature visual baseline: the accepted violet egg-shaped `seed`／`memory-seed-egg-v1` body now uses the `procedural-svg-v2` layered runtime renderer, with the accepted concept's more juvenile compact face proportions, synchronized paired eyes, lower memory core, and cheek-hugging side plates; the required access-off truth remains quiet inline text, while AI concept art and PNG icons are reference／packaging material only and later stages remain reference-only
