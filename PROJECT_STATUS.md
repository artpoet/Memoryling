...
- Creature visual baseline: the accepted violet `seed`／legacy-compatible `memory-seed-egg-v1` body now uses the `procedural-svg-v3` layered runtime renderer, with the concept's pointed upper-teardrop／broad rounded-lower-arc shell, juvenile face proportions, original-size synchronized eyes with restored breathing room, prominent memory core, and cheek-hugging plates that meet above rather than define the rounded base; the required access-off truth remains quiet inline text, while AI concept art and PNG icons are reference／packaging material only and later stages remain reference-only
