...
- Creature visual baseline: the accepted violet egg-shaped `seed`／`memory-seed-egg-v1` body now replaces the drifting CSS square-round mascot through a layered, runtime React SVG renderer; AI concept art and PNG icons are reference／packaging material only, while later stages and variants remain reference-only
