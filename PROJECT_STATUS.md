...
- Creature visual baseline: the accepted violet `seed`／legacy-compatible `memory-seed-egg-v1` body now uses the `procedural-svg-v4` layered runtime renderer, with a broader pointed upper-teardrop／rounded-lower-arc shell, original-size synchronized eyes at the wider `x=92／148` spacing, a prominent memory core, and cheek-hugging plates that meet above rather than define the rounded base; the required access-off truth remains quiet inline text, while AI concept art and PNG icons are reference／packaging material only and later stages remain reference-only
