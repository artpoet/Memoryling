...
- Creature visual baseline: the accepted violet egg-shaped `seed`／`memory-seed-egg-v1` body now replaces the drifting CSS square-round mascot through a layered, runtime React SVG renderer; paired eyes share one baseline and synchronized blink, while the required access-off truth is quiet inline text rather than a button-like pill; AI concept art and PNG icons are reference／packaging material only, while later stages and variants remain reference-only
