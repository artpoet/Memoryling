...
Memoryling 是為 AI Agent 使用者打造的開源、local-first 伴隨層。寵物是它的情感介面，而不是產品的全部：Memoryling 會把目前 Agent 原本已獲授權的脈絡，轉化為可追溯的成長、多樣的雙語對話與有限度的反思，再由確定性的 Windows App 管理持久狀態、時機、使用者控制與刪除。

先安裝並打開 Windows App，再回到已設定的專案說：

> **醒來吧我的寵物**

寵物打開後會自己顯示這句發動提醒。目前的 Agent 只使用它原本獲准讀取的脈絡，編譯成一份小型寵物更新，提交給已開啟的 App，並等待本機套用完成；接著由 Memoryling 持續管理外觀、雙語對話、冷卻、期限、安靜時段與每日額度。

除了陪伴之外，未來可選擇啟用的 Daily Memory Scout，預計只使用經使用者檢視、最小化的工作訊號，尋找有來源且與目前任務相關的更新，再提出實用建議。這項功能在 v0.7.0 尚未啟用，核心寵物不需要 API key，也能完全在本機運作。

[English](README.md) · [產品願景](docs/PRODUCT_VISION.md) · [架構](docs/ARCHITECTURE.md) · [隱私](docs/PRIVACY_PRINCIPLES.md)

## 它有什麼不同

- **是伴隨層，不是另一個聊天機器人：** 寵物透過成長、對話與反思，讓 Agent 的長期連續性變得可見、可親近。
- **由 Agent 運作：** 語意理解留在使用者原本選擇的 Agent。
- **清楚的安裝版入口：** 使用者正常打開 EXE；寵物會在首次啟動顯示 Agent 發動語與一鍵複製按鈕，待機時也會繼續提醒。
...
- **不只陪伴，也能提供實用價值：** 明確 opt-in 的未來路線會從最小化訊號提供有來源、與任務相關的建議，但不會讓雲端 AI 成為必要條件。
