...
Memoryling is pre-release software. Source v0.7.0 must not yet be trusted with production-sensitive Agent context, and it has no accepted v0.7.0 installer or private-memory UAT.
...
## Current v0.7.0 core boundary

- The user invokes one bounded update through `醒來吧我的寵物` or `Memoryling, wake up` in an Agent project. Project／Agent wake-up requests do not grant this authorization.
...
- Before reading memory for the pet workflow, the Agent checks that a running `Memoryling.exe` has product identity `Memoryling` and version 0.7.0 or newer. Submission repeats the check and waits for bounded local application confirmation.
...
- SQLite retains only the newest semantic operation, unchanged-card counters, and current／pending appearance. New apply transactionally retires obsolete content; the detail clear control removes all derived state immediately.
...
Older fixture import, exact Codex work-record, direct Codex-memory, and BYOK Daily Scout implementations remain in source for compatibility research. Direct memory and Daily Scout schedulers are disabled, and their UI is not part of the v0.7.0 core.
