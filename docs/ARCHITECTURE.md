...
Permanent growth may substantially change morphology. Distant stages do not need to remain immediately recognizable as the same form, but every adjacent stage transition must produce a versioned `EvolutionBridge` describing preserved, grown, split, merged, relocated, or retired traits. Growth is driven only by approved memories and local derivations; raw record count and calendar time are not XP. No live Agent monitoring exists. Any future `LiveAgentPresence` source requires a separate product／privacy decision, explicit consent, and a source-specific read-only adapter. Its output is an allowlisted content-free enum kept only in memory with a TTL; it does not enter SQLite, logs, telemetry, contribution records, or permanent growth. Neutral is the default, and disabling or losing the adapter clears the state immediately. If any live usage signal is ever meant to affect permanent growth, it must first become an explicitly previewed, approved, versioned event with lineage.
