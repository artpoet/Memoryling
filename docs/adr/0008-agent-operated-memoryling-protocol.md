...
1. The primary trigger is the exact pet-specific phrase `寵物醒來` or `Memoryling, wake up` in an Agent project. Reading a wake-up file or waking the project／Agent／self does not trigger the pet workflow.
