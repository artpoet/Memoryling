...
The `memory-seed-egg-v1` body-module value is retained as a legacy-compatible state identifier; it no longer describes the accepted outer silhouette. The desktop body is generated at runtime by the layered React SVG renderer in `src/ProceduralMemorySeed.tsx` (`procedural-svg-v5`). Its shell, translucent facet planes, sprouts, eyes, memory core, lower plates, plate thickness edge, highlights, and motion remain individually addressable code layers; the desktop pet does not load a raster creature image. The v5 silhouette and face proportions are grounded in the accepted AI concept reference, but every visible production layer remains deterministic program-generated SVG.

The canonical shell is not a symmetric egg. Its center rises to a visible point between the sprouts, the upper shoulders flow downward like a teardrop, and the lower body widens before closing in a soft rounded arc. Renderer v5 retains the 154-unit shell width and adds four low-opacity planes that imply organic crystal refraction without turning the creature into a hard gemstone. The paired eyes are a single synchronized motion unit: their primary ellipses share one vertical center, sit at `x=92／148` with the original `rx=10／ry=14` size in the 240-unit viewBox, and blink together. The four plates hug the cheeks, but the two inner lower plates are deliberately not mirrored flat: the right plate sits slightly behind and lower, the left plate overlaps in front, and one narrow dark edge exposes their thickness. This overlap must stay subtle and remain above the shell's rounded base; the shell—not the plates—defines the lowest visible contour. Do not combine inward eye movement with larger eye ellipses, narrow the body back into a tall capsule, raise facet contrast into a crown-like hard surface, flatten the lower plates into perfect symmetry, exaggerate their offset into a hole or broken shell, round off the shell apex into an egg, pull the plate seam down into a body-defining point, reintroduce independent eye timing, or detach the lower petals. On the compact pet surface, the required memory-access-off truth remains visible as quiet inline text rather than a pill, button, card, shadow, or other interactive-looking control.
