...
The `memory-seed-egg-v1` body-module value is retained as a legacy-compatible state identifier; it no longer describes the accepted outer silhouette. The desktop body is generated at runtime by the layered React SVG renderer in `src/ProceduralMemorySeed.tsx` (`procedural-svg-v4`). Its shell, sprouts, eyes, memory core, lower plates, highlights, and motion remain individually addressable code layers; the desktop pet does not load a raster creature image. The v4 silhouette and face proportions are grounded in the accepted AI concept reference, but every visible production layer remains deterministic program-generated SVG.

The canonical shell is not a symmetric egg. Its center rises to a visible point between the sprouts, the upper shoulders flow downward like a teardrop, and the lower body widens before closing in a soft rounded arc. Renderer v4 widens the shell from 142 to 154 viewBox units without changing its height, and the shadow, highlights, side plates, and inner plates expand with it. The paired eyes are a single synchronized motion unit: their primary ellipses share one vertical center, sit at `x=92／148` with the original `rx=10／ry=14` size in the 240-unit viewBox, and blink together. The retained eye breathing room, shorter eye-to-mouth gap, prominent memory core, outward leaf sprouts, and four continuous plates that hug the cheeks and meet just above the shell's rounded base are deliberate juvenile／cute proportions from the accepted concept. The shell—not the plates—must define the lowest visible contour. Do not combine inward eye movement with larger eye ellipses: that removes the central breathing room and makes the face feel crowded. Also do not narrow the body back into a tall capsule, round off the shell apex into an egg, invert the silhouette into a round top and pointed bottom, pull the plate seam down into a body-defining point, reintroduce independent eye timing, per-eye transforms, asymmetrical offsets, or detached necklace-like lower petals. On the compact pet surface, the required memory-access-off truth remains visible as quiet inline text rather than a pill, button, card, shadow, or other interactive-looking control.
